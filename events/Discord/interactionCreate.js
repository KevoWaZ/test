...
module.exports = async (client, inter) => {
    ...
    if (inter.type === InteractionType.ApplicationCommand) {
        ...
        if (!command) {
            errorEmbed.setDescription(await Translate('<❌> | Erreur, regarde ton code nan?'));
        ...
        if (command.permissions && !inter.member.permissions.has(command.permissions)) {
            errorEmbed.setDescription(await Translate(`<❌> | Ta pas la permission d'executer cette commande victime`));
        ...
        if (DJ.enabled && DJ.commands.includes(command) && !inter.member._roles.includes(inter.guild.roles.cache.find(x => x.name === DJ.roleName).id)) {
            errorEmbed.setDescription(await Translate(`<❌> | Cette commande est reservé aux <\`${DJ.roleName}\`> `));
            return inter.editReply({ embeds: [errorEmbed], ephemeral: true });
        }

        if (command.voiceChannel) {
            if (!inter.member.voice.channel) {
                errorEmbed.setDescription(await Translate(`<❌> | Mais tes pas dans un salon tes bete`));
                return inter.editReply({ embeds: [errorEmbed], ephemeral: true });
            }

            if (inter.guild.members.me.voice.channel && inter.member.voice.channel.id !== inter.guild.members.me.voice.channel.id) {
                errorEmbed.setDescription(await Translate(`<❌> | Rejoins le bon salon déjà`));
                return inter.editReply({ embeds: [errorEmbed], ephemeral: true });
            }
        }
