...
module.exports = {
    ...
    async execute({ inter }) {
        ...
        if (channel.type !== ChannelType.GuildText) return inter.editReply({ content: await Translate(`Ecris dans un salon text <❌>`) });

        const embed = new EmbedBuilder()
            .setTitle(await Translate('Controle la musique avec les boutons'))
            .setImage(inter.guild.iconURL({ size: 4096, dynamic: true }))
            .setColor('#2f3136')
            .setFooter({ text: await Translate('Musique'), iconURL: inter.member.avatarURL({ dynamic: true }) });

        inter.editReply({ content: await Translate(`J'envoi a <${channel}>... <✅>`) });
